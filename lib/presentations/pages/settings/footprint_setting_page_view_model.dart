...
import 'package:econa_mobile_app/presentations/pages/settings/footprint_setting_page_state.dart';
import 'package:riverpod_annotation/riverpod_annotation.dart';

part 'footprint_setting_page_view_model.g.dart';

@riverpod
class FootprintSettingPageViewModel extends _$FootprintSettingPageViewModel {
  @override
  Future<FootprintSettingPageState> build() async {
    final response = await ref.read(userRepositoryProvider).getUserSettings(
      GetUserSettingsRequest(
        settingKeys: [
          UnifiedSettingKey.UNIFIED_SETTING_KEY_LEAVE_FOOTPRINT,
        ],
      ),
    );
    ...
    return FootprintSettingPageState(
      enableFootprint: isEnabled,
    );
  }

  Future<void> updateFootprintSetting(bool isEnabled) async {
    final currentState = state.valueOrNull;
    if (currentState == null) return;

    state = AsyncData(currentState.copyWith(enableFootprint: isEnabled, error: null));

    try {
      await ref.read(userRepositoryProvider).updateUserSettings(
        UpdateUserSettingsRequest(
          settingKey: UnifiedSettingKey.UNIFIED_SETTING_KEY_LEAVE_FOOTPRINT,
          flag: isEnabled,
        ),
      );
    } catch (e) {
